...
} = require("./mutationPushTransactionEmoneyOrder.js");

describe("Create EMONEY deals", function () {
    this.timeout(175000);
    ...
    let volume, insurance, serializedTransactionJson, walletNumber
    ...
    it("Successfully create a buy deal", async () => {
        ...
        assert.equal(myDeal.wallet_number, myOrder.wallet_number)
        ...
        assert.equal(myDeal.payment_system_id, myOrder.payment_system_id)
    ...
    it("Successfully create a sell deal", async () => {
        ...
        assert.equal(myDeal.card_number, null)
        assert.equal(myDeal.payment_url, null)
        assert.equal(myDeal.wallet_number, walletNumber)
        assert.equal(myDeal.wallet_address, null)
        assert.equal(myDeal.memo, myOrder.memo)
        assert.equal(myDeal.rate, myOrder.rate)
        assert.deepEqual(myDeal.bank_id, null)
    ...
    it("pushTransaction EMONEY SELL, but wallet_number has the wrong format", async () => {
        orderType = 'BUY'
        dealType = 'SELL'
        walletNumber = 14705909234567898
    ...
    it("pushTransaction EMONEY BUY, but there is no such order in the DB", async () => {
        orderType = 'SELL'
        dealType = 'BUY'
        let orderId = 1
