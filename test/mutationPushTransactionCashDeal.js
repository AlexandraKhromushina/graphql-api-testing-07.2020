...
} = require("./mutationPushTransactionCashOrder.js");

describe("Create CASH deals", function () {
    this.timeout(175000);
    ...
    let transactionJson = []
    let paymentType, currencyId, currencyName, tokenName, countryId, countryName, isCashRate, rate
    let memo, volume, insurance, minPackage, maxPackage, serializedTransactionJson
    let generatedSignature, myOrder, dealsArray, myDeal, queryAllDealsBuyer, queryAllDealsSeller
    let dataJson, orderType, dealType, mutationPushTransaction, data, transactionName
    ...
    it("Successfully create a buy deal", async () => {
        ...
        assert.equal(myDeal.city_id, myOrder.city_id)
    ...
    it("pushTransaction CASH BUY, but payload isn't empty", async () => {
        orderType = 'SELL'
        dealType = 'BUY'
    ...
    it("pushTransaction CASH SELL, but payload isn't empty", async () => {
        orderType = 'BUY'
        dealType = 'SELL'
    ...
    it("pushTransaction CASH BUY, but there is no such order in the DB", async () => {
        orderType = 'SELL'
        dealType = 'BUY'
        let orderId = 1
