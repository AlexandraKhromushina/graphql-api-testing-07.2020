...
} = require("./mutationPushTransactionUrlOrder.js");

describe("Create URL deals", function () {
    this.timeout(175000);
    ...
    let volume, insurance, serializedTransactionJson, paymentUrl
    ...
    beforeEach(async () => {
        ...
        countryId = 236
        countryName = 'Ukraine'
        paymentUrl = 'https://youtu.be/t0xELMdGYeU'
        volume = "1.00000"
    ...
    it("Successfully create a buy deal", async () => {
        ...
        assert.equal(myDeal.payment_url, myOrder.payment_url)
        ...
        assert.deepEqual(myDeal.bank_id, null)
    ...
    it("Successfully create a sell deal", async () => {
        ...
        assert.equal(myDeal.is_paid, false)
        assert.equal(myDeal.card_number, null)
        assert.equal(myDeal.payment_url, paymentUrl)
        assert.equal(myDeal.wallet_number, null)
        assert.equal(myDeal.wallet_address, null)
        assert.equal(myDeal.memo, myOrder.memo)
        assert.equal(myDeal.rate, myOrder.rate)
    ...
    it("pushTransaction CE SELL, but payment_url has the wrong format", async () => {
        orderType = 'BUY'
        dealType = 'SELL'
        paymentUrl = 14705909234567898
