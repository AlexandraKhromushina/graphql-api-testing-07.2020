...
describe("Create URL orders", function () {
    this.timeout(175000);
    ...
    let transactionJson = []
    let paymentType, currencyId, currencyName, tokenName, countryId, countryName, isCashRate, rate, paymentUrl
    ...
    const createTransactionJson = () => {
        ...
        transactionJson = [{
            ...
            data: {
                ...
                meta: "{\"params\":{\"payment_url\":\"" + paymentUrl + "\"},\"type\":\"" + paymentType + "\",\"uuid\":\"" + uuid + "\"}",
    ...
    beforeEach(async () => {
        ...
        rate = '1.1'
        isCashRate = true
        paymentUrl = 'https://slain.in/vain'
        memo = 'memo1'
        volume = "1.00000"
    ...
    it("Successfully create a sell order", async () => {
        ...
        assert.equal(myOrder.currency_id, currencyId)
        assert.equal(myOrder.card_number, null)
        assert.equal(myOrder.payment_url, paymentUrl)
        assert.equal(myOrder.wallet_number, null)
        assert.equal(myOrder.wallet_address, null)
    })

    it("pushTransaction URL SELL with the payload of BUY", async () => {
        orderType = 'BUY'
    ...
    it("pushTransaction URL BUY with the payload of SELL", async () => {
        orderType = 'SELL'
    ...
    it("pushTransaction URL BUY but rate has the wrong format", async () => {
        orderType = 'BUY'
        rate = 3
    ...
    it("pushTransaction URL BUY but currency_id has the wrong format", async () => {
        orderType = 'BUY'
        currencyId = '2'
    ...
    it("pushTransaction URL BUY but currency_id is way too big", async () => {
        orderType = 'BUY'
        currencyId = 92
    ...
    it("pushTransaction URL BUY but country_id has the wrong format", async () => {
        orderType = 'BUY'
        countryId = '16'
    ...
    it("pushTransaction URL BUY but country_id is way too big", async () => {
        orderType = 'BUY'
        countryId = 3456
    ...
    it("pushTransaction URL SELL but payment_url has the wrong format", async () => {
        orderType = 'BUY'
        paymentUrl = 3456
        ...
        console.log(response)
        assert.equal(response.pushTransaction.transactionResponse, null)
        assert.equal(response.pushTransaction.response.includes('\'payment_url\': [\'Not a valid string.\']'), true)
        assert.equal(response.pushTransaction.errors, 'INVALID_DATA')
    })

    it("pushTransaction URL SELL but memo has the wrong format", async () => {
        orderType = 'BUY'
        memo = 3456
    ...
    it("pushTransaction URL SELL but is_cash_rate has the wrong format", async () => {
        orderType = 'BUY'
        isCashRate = '0'
    ...
    it("pushTransaction URL SELL but the signature is incorrect", async () => {
        orderType = 'SELL'
    ...
    it("pushTransaction URL SELL but there is an error from blockchain", async () => {
        orderType = 'SELL'
        volume = "1.0000"
